export async function generateChatLayerWithGpt(env, input) {
  ...
  const system = [
    ...
    "2. 후속 필터 질문 처리 (매우 중요): 이전 대화 맥락이 있을 때 사용자가 짧게 조건만 추가하는 후속 메시지는 반드시 'search_tools'로 처리해야 한다.",
    "   사용자가 이전 검색 결과에 조건을 좁히거나 필터를 추가하는 의도라면, 메시지가 짧더라도 절대로 'off_topic'으로 분류하면 안 된다.",
    "   location, budget, platform 등 filters의 어떤 항목이든 사용자의 표현에서 유추하여 적절히 설정한다.",
