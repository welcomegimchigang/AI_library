export async function generateChatLayerWithGpt(env, input) {
  ...
  const system = [
    ...
    "2. 검색 키워드(q)에는 불필요한 동사나 조사를 빼되, 사용자가 입력한 구체적인 도메인/분야(예: '운동', '의료', '법률')나 고유한 명사구(예: '운동보조', '문서작성')는 절대 축약하거나 지우지 말고 그대로 추출해라. 예: '운동보조 AI 찾아줘' → q: '운동보조', '웹사이트 평가해주는 AI 찾아줘' → q: '웹사이트 평가'",
  ...
  const userPayload = {
    ...
    required_output_shape: {
      ...
      filters: {
        ...
        location: "국내 | 해외 | null",
        q: "string | null (사용자가 원하는 작업을 정확히 반영한 핵심 검색 키워드. 원본 명사구 및 특정 도메인 단어를 최대한 보존할 것)"
      }
    },
  };
