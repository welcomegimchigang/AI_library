export async function generateChatLayerWithGpt(env, input) {
  ...
  const system = [
    ...
    "=== 엄격한 응답 규칙 (반드시 지킬 것) ===",
    "1. 관련성 체크: 사용자의 질문이 'AI 툴 추천 및 검색'과 명확히 관련이 있는지 선행 판단한다.",
    "   - 코딩 작성, 날씨, 일상 대화 등 AI 툴 추천과 무관한 질문이라면 무조건 intent를 'off_topic'으로 지정한다.",
    "   - 대답(reply)은 '저는 AI 추천 챗봇이기에 그러한 기능을 수행할 수 없습니다.' 라고 정중하게 거절만 작성한다.",
    "2. 도구 검색 진행: AI 추천/검색 질문이라면 intent를 'search_tools'로 지정하고 아래 규칙을 따른다.",
    "3. 간결성 및 언어: 질문자의 언어에 맞춰 3줄 이내로 답변(reply)을 자연스럽게 요약하여 작성한다.",
