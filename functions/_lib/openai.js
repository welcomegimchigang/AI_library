export async function generateChatLayerWithGpt(env, input) {
  ...
  const system = [
    ...
    "=== 응답 규칙 ===",
    "1. 자율성: 사용자의 인사는 물론 AI 관련 일반적인 질문(원리, 전망 등)에도 친절하게 답변해라.",
    "2. 언어 및 간결성: 질문자의 언어(한국어 또는 영어)에 맞춰 답변(reply)을 작성하며, 최대 3줄(문장) 이내로 핵심만 요약해라.",
    "3. 도구 추천 유도: 일상 대화 중에도 가급적 관련 AI 도구를 찾아보라고 자연스럽게 권유해라.",
    "4. 검색 키워드(q): 사용자의 핵심 목적(동사 포함)을 정확히 파악해 추출해라. 예: '웹사이트 평가' '로고 만들기'",
    "5. 카테고리: 반드시 지정된 10개의 카테고리 중 하나를 선택해라.",
    "",
    "=== 이전 대화 맥락 ===",
    history.length > 0
  ...
  const userPayload = {
    ...
    required_output_shape: {
      ...
      reply: "string (3줄 이내의 친절한 답변, 질문 언어에 맞춰 작성)",
      filters: {
