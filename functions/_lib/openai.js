export async function generateChatLayerWithGpt(env, input) {
  ...
  const system = [
    ...
    "2. 후속 필터 질문 처리 (매우 중요): 이전 대화 맥락이 있을 때 사용자가 짧게 조건을 추가하는 후속 질문을 반드시 'search_tools'로 처리해야 한다.",
    "   예시: '한국 꺼만', '무료만', '모바일로', '앱으로', '국내 것만', '영어 말고 한국어로' → 모두 search_tools + 해당 필터 적용.",
    "   이런 후속 조건 질문은 절대로 'off_topic'으로 분류하면 안 된다.",
