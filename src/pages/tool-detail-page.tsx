...
import { ArrowLeft, ExternalLink, ThumbsUp, Star, MessageSquare, Bookmark, BookmarkCheck } from "lucide-react";
...
export function ToolDetailPage() {
    ...
    let hostname = "";
    try { if (tool) hostname = new URL(tool.url).hostname; } catch { }
        ...
        return (
            <div className="min-h-screen bg-slate-50">
                ...
                <div className="text-center py-20">
                    ...
                    <Link to="/" className="mt-4 inline-block text-blue-600 hover:underline">홈으로 돌아가기</Link>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-[radial-gradient(120%_80%_at_10%_0%,#dbeafe_0%,#e9d5ff_40%,#fce7f3_75%,#f8fafc_100%)] text-slate-900">
            ...
            <main className="mx-auto w-full max-w-4xl px-4 py-10 md:px-6">
                <Link to="/#library" className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-slate-700 mb-6">
                ...
                <div className="bg-white rounded-3xl shadow-xl overflow-hidden">
                    ...
                    <div className="bg-slate-50 p-8 flex items-center gap-6 border-b border-slate-100">
                        <div className="w-20 h-20 rounded-2xl border border-slate-200 bg-white p-2 flex items-center justify-center overflow-hidden shadow-sm">
                            {hostname ? (
                                ...
                                <span className="text-3xl font-black text-slate-300">{tool.name.charAt(0)}</span>
                            )}
                        </div>
                        <div className="flex-1">
                            <div className="flex items-center gap-3 mb-1">
                                <h1 className="text-2xl font-bold">{tool.name}</h1>
                                <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold text-white ${tool.isFree ? 'bg-emerald-500' : 'bg-rose-500'}`}>
                                    {tool.isFree ? '무료' : '유료'}
                                </span>
                    ...
                    <div className="p-8">
                        ...
                        <div className="flex flex-wrap gap-3 mt-8">
                            ...
                            <Button size="lg" variant="outline" onClick={handleUpvote} className="text-slate-700 border-slate-300">
                                <ThumbsUp size={16} className="mr-2" /> 추천 {upvotes}
                            </Button>
                            <Button size="lg" variant="outline" onClick={toggleBookmark} className="text-slate-700 border-slate-300">
                                {isBookmarked ? <BookmarkCheck size={16} className="mr-2 text-blue-600" /> : <Bookmark size={16} className="mr-2" />}
                                {isBookmarked ? "저장됨" : "북마크"}
                            </Button>
                            <Button size="lg" variant="outline" onClick={() => {
                                if (!getUserSession()) { alert("로그인이 필요합니다."); return; }
                                setShowReview(true);
                ...
                {relatedTools.length > 0 && (
                    <div className="mt-10">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {relatedTools.map(rt => {
                                let rh = "";
                                try { rh = new URL(rt.url).hostname; } catch { }
                                return (
                                    <Link key={rt.id} to={`/tool/${rt.id}`} className="bg-white rounded-xl p-4 border border-slate-200 hover:shadow-md transition-shadow flex items-start gap-3">
                                        <div className="w-10 h-10 rounded-lg bg-slate-50 border border-slate-100 flex items-center justify-center overflow-hidden flex-shrink-0">
                                            {rh ? (
                                                <img src={`https://www.google.com/s2/favicons?domain=${rh}&sz=64`} alt={rt.name} className="w-7 h-7" />
                                            ) : (
                                                <span className="text-sm font-bold text-slate-400">{rt.name.charAt(0)}</span>
                                            )}
                                        </div>
                                        <div className="min-w-0">
                                            <h3 className="font-bold text-sm text-slate-900 truncate">{rt.name}</h3>
                                            <p className="text-xs text-slate-500 line-clamp-2 mt-0.5">{rt.description}</p>
                                        </div>
                                    </Link>
                                );
                            })}
